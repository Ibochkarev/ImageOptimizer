# Настройки ImageOptimizer

Все ключи в namespace `imageoptimizer`. Редактирование: **ImageOptimizer → Настройки** или **Система → Настройки системы** (фильтр `imageoptimizer`).

## Общие

| Ключ | Тип | По умолчанию | Описание |
|------|-----|--------------|----------|
| `imageoptimizer_enabled` | boolean | `1` | Глобальный выключатель: очередь, конвертация, инъекция |
...
| Ключ | Тип | По умолчанию | Описание |
|------|-----|--------------|----------|
| `imageoptimizer_convert_on_upload` | boolean | `1` | Постановка в очередь (и частично sync) при загрузке в File Manager |
...
| `imageoptimizer_avif_enabled` | boolean | `0` | Отдельный флаг для AVIF (дополнительно к formats) |
...
| `imageoptimizer_method_priority` | text | `cwebp,gd,imagick` | Порядок энкодеров (также `avifenc` для AVIF) |
...
| Ключ | Тип | По умолчанию | Описание |
|------|-----|--------------|----------|
| `imageoptimizer_cron_limit` | number | `200` | Задач за один запуск `cron/convert.php` |
...
| `imageoptimizer_inject_email` | boolean | `0` | То же для HTML писем (если контекст поддерживается) |
| `imageoptimizer_html_cache` | boolean | `1` | Кэш результата в `core/cache/imageoptimizer/html/` |
| `imageoptimizer_max_html_size` | number | `1048576` | Не обрабатывать HTML больше N байт |
| `imageoptimizer_default_sizes` | textarea | `(min-width: 1280px) 50vw, 100vw` | `sizes`, если не задан на `<img>` |
| `imageoptimizer_skip_classes` | text | `lazy,swiper-lazy,no-optim` | Классы на `<img>` через запятую — skip |
| `imageoptimizer_skip_src_pattern` | text | `thumb3x` | Подстрока в `src` — skip (Thumb3x URL) |
...
| `imageoptimizer_skip_lazy_first_images` | number | `0` | Первые N `<img>` на странице — `loading=eager` |

## Рекомендуемые пресеты

### Каталог интернет-магазина (MiniShop3)
...
Атрибуты для кастомизации (не skip):

- `data-imageoptimizer-sizes` — override `sizes` для этого img
- `data-imageoptimizer-fetchpriority` — `high` / `low` для LCP-картинок

## Связанные документы

- [frontend-guide.md](frontend-guide.md) — поведение инъекции
- [manager-guide.md](manager-guide.md) — UI настроек
- [cli.md](cli.md) — override breakpoints/format из CLI
