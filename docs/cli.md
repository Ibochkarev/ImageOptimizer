...
Пути ниже относительно корня MODX (`core/components/imageoptimizer/...`).
...
| `--path=PATH` | Подкаталог для `--scan`, **или** один файл/каталог для enqueue (`imageoptimizer_rebuild_path`) |
...
| `--time-budget=300` | Лимит времени в секундах (по умолчанию из helper) |
...
Удаляет из таблицы `imageoptimizer_queue` записи со статусом `done` старше `imageoptimizer_retention_days`:

```bash
0 3 * * * php /var/www/site/core/components/imageoptimizer/cron/prune.php
```

При `retention_days=0` prune не удаляет строки (проверьте код/log при настройке).
...
- [installation.md](installation.md) — первый cron после install
- [configuration.md](configuration.md) — `cron_limit`, `stuck_minutes`, `retention_days`
- [manager-guide.md](manager-guide.md) — очередь в UI
